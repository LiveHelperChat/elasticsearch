		...
		<?php include(erLhcoreClassDesign::designtpl('elasticsearch/parts/filter.tpl.php')); ?>
	   	<?php if ($tab == 'chats') : ?>
            <?php if (isset($pages) && $pages->items_total > 0): ?>
            	<table class="table table-sm mt-1 list-links" id="chat-list-table">
            		<thead>
            			<tr>
            		...
            		<?php foreach ($items as $item) : ?>
            		    <tr data-chat-id="<?php echo $item->chat_id?>" class="chat-row-tr" id="chat-row-tr-<?php echo $item->chat_id?>">
                            ...
                            <?php foreach ($chat->aicons as $aicon) : ?>
                                ...
                                <?php else : ?>
                                    <i class="material-icons" style="color: <?php isset($aicon['c']) ? print htmlspecialchars($aicon['c']) : print '#6c757d'?>" title="<?php isset($aicon['t']) ? print htmlspecialchars($aicon['t']) : htmlspecialchars($aicon['i'])?>"><?php isset($aicon['i']) ? print htmlspecialchars($aicon['i']) : htmlspecialchars(is_string($aicon) ? $aicon : json_encode($aicon))?></i>
                                <?php endif; ?>
                            <?php endforeach; ?>
                            ...
                            <?php else : ?>
                                ...
                                <a class="action-image material-icons" data-title="<?php echo htmlspecialchars($item->nick,ENT_QUOTES);?>" onclick="<?php if (isset($itemsArchive[$item->chat_id]) && $itemsArchive[$item->chat_id]['archive'] == true) : ?>lhinst.startChatNewWindowArchive('<?php echo $itemsArchive[$item->chat_id]['archive_id']?>','<?php echo $item->chat_id;?>',$(this).attr('data-title'))<?php else : ?>lhinst.startChatNewWindow('<?php echo $item->chat_id;?>',$(this).attr('data-title'))<?php endif;?>" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/pendingchats','Open in a new window');?>">open_in_new</a>
                                <a class="chat-link" <?php if ($chat->nc != '') : ?>style="color: <?php echo htmlspecialchars($chat->nc)?>"<?php endif;?> href="#/chat-id-<?php echo $item->chat_id?>"><?php echo $item->chat_id?></a>
                            <?php endif; ?>
            		        ...
            		        <td nowrap="nowrap" ng-non-bindable <?php if ($chat->nc != '') : ?>style="color: <?php echo htmlspecialchars($chat->nc)?>"<?php endif;?> <?php if ($chat->nb == 1) : ?>class="fw-bold"<?php endif;?> >
                                ...
                                <span class="text-muted fs11 ms-1" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/lists','Messages count');?>"><span class="material-icons">message</span><?php echo (int)$chat->all_msg_count?></span>
                            </td>
                            <td ng-non-bindable>
